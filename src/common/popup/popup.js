...
function displayCopyPanel(ticket) {
  ...
  gitBranch = cleanTitleForGitBranch(ticket.type, ticket.id + '-' + ticket.title);
...
function cleanTitleForGitBranch(storyType, title) {
  ...
  translate = { 'ä': 'ae', 'ö': 'oe', 'ü': 'ue', 'ß': 'ss' };
  cleanedTitle = storyType + '/' + title.toLowerCase().replace(/[öäüß]/g, function (match) {
